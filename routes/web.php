...
// Cart section
    ...
    Route::get('/cart', function () {
        ...
        if(Auth::check()) {
            ...
            $order = \App\Models\Order::where('user_id', $user_id)->get();
            foreach ($order as $valueAmount) {
                $totalAmount += $valueAmount->total_amount;
            }
            if ($totalAmount == 100000.00){
                $sale = 10/100;
            } elseif ($totalAmount >= 100000.00){
                $sale = 20/100;
            } else {
                $sale = 0;
            }
        }
        return view('frontend.pages.cart', compact('sale'));
    })->name('cart');
...
// Wishlist
    ...
    Route::get('/createpaypal', function (){
...
// Payment
    Route::get('create-paypal/{id}',[PaypalController::class,'processPaypal'])->name('createPaypal.processPaypal');
